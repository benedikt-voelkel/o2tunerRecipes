...
def run_reference(inspectors, config):
    """
    ...
    reference_params = []
    params = parse_json(o2_medium_params)
...
def run_baseline(inspectors, config):
    ...
    cmd = f'o2-sim-serial -n {config["events"]} -g extkinO2 --extKinFile {kine_file} -e MCReplay ' \
          f'--configKeyValues="MCReplayParam.stepFilename={steplogger_file};MCReplayParam.blockProcessesCuts=false;MCReplayParam.allowStopTrack=false"'
    run_command(cmd, log_file=config["o2_sim_log"])
    return True
